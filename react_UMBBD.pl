...
open (SIFILE, ">testingreaction records.sif") or die "Can't open file for write $!\n";
...
my $reactionID; my $enz_name;my $brk = "PLUS";my @prd;my @sbrt; my $enzyme;
	if ($line =~ /.+reacID=(.+)\">(.+) -----&gt (.+) \(reacID\#/){
		...
		if (($substrate !~ /PLUS/) && ($product =~ /PLUS/)){
			...
			print SIFILE "$reactionID\t$substrate\t$enz_name\t$prd[0]\n";
			print SIFILE "$reactionID\t$substrate\t$enz_name\t$prd[1]\n";
			print "$reactionID\t$substrate\t$enz_name\t$prd[0]\n";
			print "$reactionID\t$substrate\t$enz_name\t$prd[1]\n";
			}
		if (($substrate =~ /PLUS/) && ($product !~ /PLUS/)){
			@sbrt = split(/PLUS/, $substrate);
			print SIFILE "$reactionID\t$sbrt[0]\t$enz_name\t$product\n";
			print SIFILE "$reactionID\t$sbrt[1]\t$enz_name\t$product\n";
			print "$reactionID\t$sbrt[0]\t$enz_name\t$product\n";
			print "$reactionID\t$sbrt[1]\t$enz_name\t$product\n";
			}
		if (($substrate !~ /PLUS/) && ($product !~ /PLUS/)){
			print SIFILE "$reactionID\t$substrate\t$enz_name\t$product\n";
			print "$reactionID\t$substrate\t$enz_name\t$product\n";
			}
	}
